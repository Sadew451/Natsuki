...
from Natsuki import dispatcher, spamcheck, LOGGER
...
from Natsuki.modules.sql import languages_sql as sql
...
for x in os.listdir('Natsuki/modules/langs'):
	if os.path.isdir('Natsuki/modules/langs/'+x):
		continue
	x = x.replace('.py', '')
	LOADED_LANGS_ID.append(x)
	imported_langs = importlib.import_module("Natsuki.modules.langs." + x)
